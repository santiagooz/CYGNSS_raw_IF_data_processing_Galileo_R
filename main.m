...
load galileo_e1b
a = sqrt(10/11);
b = 0; % b subcarrier not present due to front-end bandwidth
sc_E1B = a*sign(sin(2*pi*(1:12)/12)) + b*sign(sin(2*pi*(1:12)/2));
cod_pri_b = 2*e1b(SVID,:)-1;
c = kron(cod_pri_b,sc_E1B);
...
for batch = 1 : num_ddm
    ...
    if success ==  true
        ...
        y = coh_corr_gal( signal, fs/dec_ratio, c, Doppler, delay0, Ti, delay_initial + delay_central((batch-1)*K+1:batch*K));
...
a = linspace(0,1,num_ddm); dt1 = mode(diff(t0));
sp_lat_int = sp_lla(1,1) + (sp_lla(floor(num_ddm*K*Ti/dt1),1)-sp_lla(1,1))*a;
sp_lon_int = sp_lla(1,2) + (sp_lla(floor(num_ddm*K*Ti/dt1),2)-sp_lla(1,2))*a;

SNRdB = zeros(1, num_ddm);
SNRdBmax = 25; % max value in y axis for SNRdB plot
...
for batch = 1 : num_ddm
    ...
    delay = delay - floor(delay(1)/4096)*4096;
    if Lf > 1 % plots DDM and WF
        ...
        % [delay_max, doppler_max] = find(ddm(:,:,batch) == ddm_max);
    ...
    geoscatter(sp_lat_int(batch),sp_lon_int(batch),36,min(SNRdB(batch),SNRdBmax),'Filled');
