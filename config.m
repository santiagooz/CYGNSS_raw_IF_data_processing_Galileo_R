...
time_to_process = 3;   % total time to process
caseID = 5;     % case ID number listed in case_selector.m

% Processing and delay-Doppler plane parameters
Ti = 4e-3;      % coherent integration time [secs]
Tnc = .5;        % non-coherent integration time [secs]
dec_ratio = 2;  % decimation ratio
Doppler_resolution = 250;   % DDM Doppler bin resolution [Hz]
...
% Useful constants
fc = 1.023e6;       % Galileo E1B code chip frequency [Hz]
fL1 = 1575.42e6;    % Galileo E1 nominal carrier frequency [Hz]
...
bytes_guard = ceil(abs(min(Doppler_central+Doppler_offset))*Ti/1540*fs/fc*K/4)+1;
